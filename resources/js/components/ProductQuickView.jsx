...
export default function ProductQuickView({ product, onClose }) {
    ...
    useEffect(() => {
        // If we only have a partial product (e.g. from inventory list that has no variations preloaded)
        // or we just want to ensure we have the latest and complete details including variations
        const needsFetch = !product.variations || (product.has_variations && product.variations.length === 0);
        
        if (needsFetch && product.id) {
            setLoading(true);
            axios.get(`/api/products/${product.id}`)
                ...
                .catch(err => {
                    console.error("Failed to fetch product quick view details:", err);
                })
                .finally(() => {
                    setLoading(false);
                });
    ...
    const baseMargin = fullProduct.selling_price > 0 
        ? ((fullProduct.selling_price - fullProduct.cost_price) / fullProduct.selling_price * 100).toFixed(2)
        : '0.00';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
            <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center justify-between p-5 border-b border-slate-800">
                    <div>
                        <div className="flex items-center gap-2">
                            <h2 className="text-xl font-semibold text-slate-100">{fullProduct.name}</h2>
                            <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                                fullProduct.status === 'active' 
                                    ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' 
                                    : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
                            }`}>
                                {fullProduct.status === 'active' ? 'Active' : 'Inactive'}
                            </span>
                        </div>
                        <p className="text-xs text-slate-400 mt-1">
                            {fullProduct.category?.name || 'No Category'} &bull; {fullProduct.brand?.name || 'No Brand'}
                        </p>
                    </div>
                    <button 
                        onClick={onClose}
                ...
                <div className="flex-1 p-6 overflow-y-auto space-y-6">
                    {loading ? (
                        <div className="min-h-[260px] flex flex-col items-center justify-center text-slate-400">
                            <RefreshCw className="w-8 h-8 animate-spin text-indigo-500 mb-2" />
                            <span className="text-xs">Fetching complete product specifications...</span>
                        </div>
                    ) : (
                        <>
                                ...
                                <div className="md:col-span-1 flex items-center justify-center border border-slate-800 bg-slate-950/40 rounded-xl p-4 min-h-[200px] overflow-hidden relative">
                                    {fullProduct.image_url ? (
                                        <img 
                                            ...
                                            className="max-w-full max-h-[220px] object-contain rounded-lg animate-fade-in"
                                        />
                                    ) : (
                                        <div className="text-center text-slate-505">
                                ...
                                <div className="md:col-span-2 space-y-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-300 mt-1 whitespace-pre-line leading-relaxed">
                                            {fullProduct.description || 'No description provided.'}
                                        </p>
                                    </div>
                                    ...
                                    {!fullProduct.has_variations && (
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                                            <div className="bg-slate-950/30 border border-slate-800/80 rounded-xl p-3.5">
                                                ...
                                                <span className="text-base font-semibold text-slate-200">${fullProduct.cost_price}</span>
                                            </div>
                                            <div className="bg-slate-950/30 border border-slate-800/80 rounded-xl p-3.5">
                                                <span className="text-[11px] text-slate-400 font-medium block">Selling Price</span>
                                                <span className="text-base font-semibold text-slate-200">${fullProduct.selling_price}</span>
                                            </div>
                                            <div className="bg-slate-950/30 border border-slate-800/80 rounded-xl p-3.5">
                                                <span className="text-[11px] text-slate-400 font-medium block">Tax Rate</span>
                                                <span className="text-base font-semibold text-slate-200">{fullProduct.tax}%</span>
                                            </div>
